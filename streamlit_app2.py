...
with st.sidebar:
    ...
    # NEW: Wake up backend button
    if st.button("⚡ Wake up Backend"):
        try:
            wake_response = requests.get(f"{api_base_url}/api/v1/health", timeout=30)
            if wake_response.status_code == 200:
                st.success("✅ Backend is awake and ready!")
            else:
                st.warning(f"⚠️ Backend responded with {wake_response.status_code}, may still be waking.")
        except requests.exceptions.RequestException as e:
    ...
    st.header("ℹ️ About")
    st.markdown("""
    **TRIPPY** is your AI-powered travel planning assistant 🚀  
    It uses advanced AI to generate **personalized itineraries** tailored to your style of travel.  

    ### ✨ Key Features
    - 🌍 **Destination Planning** – Choose origin & destination effortlessly.  
    - 📅 **Smart Scheduling** – Optimized itineraries based on travel dates & duration.  
    - 🎯 **Interest Matching** – Customize plans around your hobbies (adventure, food, culture, etc.).  
    - 💰 **Budget Awareness** – Get recommendations that suit your spending range.  
    - ⚡ **Instant AI Suggestions** – Receive ready-to-use day-by-day trip itineraries in seconds.  
    - 💾 **Export Options** – Save itineraries as text or JSON for easy sharing.  
    - 🛠 **Debug Mode** – Inspect raw API responses for developers.  

    ### 🧳 Why TRIPPY?
    Traditional planning is time-consuming. TRIPPY combines **AI reasoning + travel insights** to help you:  
    - Discover new experiences.  
    - Balance relaxation & adventure.  
    - Save time while ensuring memorable trips.  
    """)
    ...
    if response_data.get('status') == 'success':
        ...
        if itinerary:
            ...
            with tab3:
                ...
                with col_download1:
                    download_text = format_itinerary(itinerary)
                    st.download_button(
